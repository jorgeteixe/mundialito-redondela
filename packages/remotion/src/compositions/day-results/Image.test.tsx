...
import { DailyResultsImage } from "./Image";

vi.mock("../../components/TeamAvatar", () => ({
  TeamAvatar: ({ name }: { name: string }) => <span>{name}</span>,
}));

describe("DailyResultsImage", () => {
  it("renders all 6 results in the square post variant", () => {
    ...
    expect(html).toContain("6 resultados");
    expect(html).not.toContain("+1 resultado");
  });
});
