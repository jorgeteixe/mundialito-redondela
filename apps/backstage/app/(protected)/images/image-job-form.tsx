...
import { createImageGenerationJob, type ImageJobFormState } from "./actions";
import type { ImageTemplateSummary } from "./data";

const initialFormState: ImageJobFormState = { status: "idle" };

type ImageJobFormProps = {
  templates: ImageTemplateSummary[];
  onSuccess?: () => void;
};

export function ImageJobForm({ templates, onSuccess }: ImageJobFormProps) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4">
      <div className="flex flex-col gap-2">
        <Label htmlFor="image-template">Plantilla</Label>
        <Select
          name="templateId"
          value={templateId}
...
function ParameterField({
  ...
  const id = `image-param-${parameter.name}`;
